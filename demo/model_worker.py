...
from sot.models.fastchat_model import FastChatModel
...
class ModelWorker(BaseModelWorker):
    def __init__(
        ...
        if router_file is not None:
            # SoT-R
            self.model_names = [
                model_name + " (SoT-R)" for model_name in self.model_names
            ]
            self.scheduler = RouterOutlineBatchScheduler(
                model=self._model,
                router_name_or_path=router_file,
            ...
            # SoT
            self.model_names = [
                model_name + " (SoT)" for model_name in self.model_names
            ]
            self.scheduler = OutlineBatchScheduler(
                prompt_file=prompt_file, model=self._model
            )
        else:
            # Normal
            self.model_names = [
                model_name + " (Normal)" for model_name in self.model_names
            ]
            self.scheduler = NaiveScheduler(model=self._model)
    ...
    def generate_stream_gate(self, params):
        ...
        self.scheduler.set_model(self._model)
        ...
        try:
            for output in self.scheduler.get_response(request, stream=True):
                ret = {
                    "text": output["text"],
                    "error_code": 0,
                }
        ...
        tim = starter.elapsed_time(ender)
        mem = torch.cuda.max_memory_allocated()

        token_size = self.count_token({"prompt": ret["text"]})["count"]

        ret["meta_info"] = (
            "Generation Time: {:.2f}s".format(tim / 1000)
            + "\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The SoT prompt file path. Default to None, meaning the normal decoding"
            " mode."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "The SoT-R router file path. Default to None, meaning not to use the router"
        ),
    )
    ...
    worker = ModelWorker(
        ...
        gptq_ckpt=args.gptq_ckpt,
        gptq_wbits=args.gptq_wbits,
        gptq_groupsize=args.gptq_groupsize,
        gptq_act_order=args.gptq_act_order,
        awq_ckpt=args.awq_ckpt,
        awq_wbits=args.awq_wbits,
        awq_groupsize=args.awq_groupsize,
        revision=args.revision,
        stream_interval=args.stream_interval,
        prompt_file=args.sot,
        router_file=args.sotr,
    )
